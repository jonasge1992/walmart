...
if uploaded_file is not None:
    ...
    merge_scaled_df = pd.DataFrame()  # Create an empty DataFrame

# Helper function to extract categories and sum sales
def get_category_sales(df):
    if 'id' in df.columns and 'sales' in df.columns:
...
if page == "Home":
    ...
    if uploaded_file is not None:
        ...
        st.write("Please upload a CSV file to see the data and make predictions.")

# Forecasting Page
elif page == "Forecasting":
    st.title("Forecasting")
    st.markdown("### Behold the sales forecast for the next 28 days ")
    if not merge_scaled_df.empty:
        st.write(merge_scaled_df.head())
        st.write("The WRMSSE has a value of: ")
        st.write("The RMSE has a value of: ")
    else:
        st.write("Please upload a CSV file to see the data and forecasts.")
...
# Sales Analysis Page
    ...
    if not merge_scaled_df.empty:
        ...
        st.write("Please upload a CSV file to see the sales analysis.")
...
# Product Analysis Page
    ...
    if not merge_scaled_df.empty:
        ...
        if selected_product_ids:
            filtered_df = merge_scaled_df[merge_scaled_df['id'].isin(selected_product_ids)]
        ...
        st.write("Please upload a CSV file to analyze products.")
...
# Interpretation Page
    ...
    if not merge_scaled_df.empty:
        ...
        fig_histogram = px.bar(sales_28_days, x='Date', y='Sales', title='Total Sales in 28-Day Intervals', labels={'Sales': 'Total Sales', 'Date': 'Date Interval'})
        ...
        st.write("Please upload a CSV file to see the data interpretation.")
